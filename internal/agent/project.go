...
func (p *ProjectAgent) Start(ctx context.Context, opts StartOptions) (StartResult, error) {
	...
	if err := github.CommentIssue(ctx, opts.Repo, opts.IssueNumber, fmt.Sprintf("🤖 max-ops agent starting work (session: `%s`)", session)); err != nil {
		return StartResult{}, err
	}
	...
	branch := fmt.Sprintf("max-ops/%d-%s", opts.IssueNumber, slugify(issue.Title))
	...
	prTitle := issue.Title
	prBody := fmt.Sprintf("Closes #%d\n\nGenerated by max-ops agent.", opts.IssueNumber)
	...
	if notifier != nil {
		_ = notifier.Notify(ctx, notifications.Message{
			Title:   "🤖 max-ops: Draft PR ready for review",
			Body:    fmt.Sprintf("PR #%d: %s\nRepo: %s\nIssue: #%d\nAuthor: max-ops agent (session %s)", prNumber, issue.Title, repoDisplay(opts.Repo), opts.IssueNumber, session),
